...
bishop_relevent_bits = np.load("BISHOP_RELEVENT_BITS.npy", allow_pickle=True)
bishop_shift = np.load("BISHOP_SHIFTS.npy", allow_pickle=True)
rook_relevent_bits = np.load("ROOK_RELEVENT_BITS.npy", allow_pickle=True)
...
from Utilities import print_bitboard


def init_slider_attacks(is_bishop: bool):
    for sq in range(64):
