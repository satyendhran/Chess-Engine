...
def parse_fen(fen: bytes):
    ...
    if len(start_fen) == len(fen) and np.all(
        ...
        return pieces, occupancy, np.uint8(0), np.uint8(0b1111), np.uint8(64)
    ...
    return pieces, occupancy, side, castle, enpassant


@jitclass(specs)  # type:ignore
class Board:
    def __init__(self, bitboard, occupancy, side, castle, enpassant):
...
def print_board(board: Board, perspective: Color):
    ...
    print("  +---+---+---+---+---+---+---+---+")
    for r in ranks:
        print((8 - r) if perspective == Color.WHITE else (r + 1), end=" |")
        for f in files:
            sq = 8 * r + f
            for piece in Pieces:
                if piece == Pieces.NONE:
                    continue
                if get_bit(board.bitboard[piece], sq):
                    print(f" {piece_sym[piece]} |", end="")
                    break
            else:
                print("   |", end="")
        print()
...
@njit
def is_square_attacked(board: Board, square: int, color: int) -> Literal[0, 1]:
    # Pawns
...
def print_attacked_squares(board: Board, color) -> None:
    for r in range(8):
        for f in range(8):
            ...
            print(is_square_attacked(board, square=square, color=color), end=" ")
        print()
    print("\n   a b c d e f g h")
